...
export default function Room(props) {
  ...
  useFrame((state) => {
    state.camera.position.lerp(
      {
        x: data.offset > 0.5 ? 5 : 7.5,
        y: data.offset > 0.5 ? 5 : 7,
        z: data.offset > 0.5 ? 5 : 7.5,
      },
      0.005
    );

    state.camera.lookAt(
      data.offset > 0.5 ? ref.current.position.x : 0,
      data.offset > 0.5 ? ref.current.position.y : 0,
      data.offset > 0.5 ? ref.current.position.z : 0
    );
  });
